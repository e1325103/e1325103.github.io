...
namespace artr
{
		...
		m_lights_editor()
	{
		...
		TwAddVarRW(m_scene_settings, "Normal Map Strength", TW_TYPE_FLOAT, &m_normal_map_strength, "min=0 max=1 step=0.01");
		...
		for (auto i = 0; i < m_point_lights.size(); ++i)
		{
			...
			light_data.position[i] = glm::vec4(m_point_lights.at(i).position(), 1.0);
			light_data.color[i] = glm::vec4(m_point_lights.at(i).light_color(), 1.0);
			light_data.attenuation[i] = m_point_lights.at(i).attenuation();
		}

		glGenBuffers(1, &lightSSBO);
		glBindBuffer(GL_SHADER_STORAGE_BUFFER, lightSSBO);
		glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(light_data), &light_data, GL_DYNAMIC_DRAW);
	...
	void A1NormalMapping::Render(const glm::mat4& pM, const glm::mat4& vM)
	{
		...
		/*for (int i = 0; i < 20; i++) {
			shader.SetLight("uPointLight[" + std::to_string(i) + "]", m_point_lights[i].GetGpuData(vM));
		}*/
	...
	void A1NormalMapping::SetUniformMatrices(Shader& shader, const glm::mat4& pM, const glm::mat4& vM, const glm::mat4& mM)
	{
		...
		shader.SetUniform("ivMatrix", glm::inverse(vM));
		shader.SetUniform("mMatrix", mM);
		shader.SetUniform("vmMatrix", vmM);
		shader.SetUniform("vmNormalMatrix", vmNrmM);
		shader.SetUniform("uNormalMappingStrength", m_normal_map_strength);

		for (auto i = 0; i < m_point_lights.size(); ++i)
		{
			light_data.position[i] = glm::vec4(m_point_lights.at(i).position(), 1.0);
			if (m_point_lights.at(i).enabled()) {
				light_data.color[i] = glm::vec4(m_point_lights.at(i).light_color(), 1.0);
			}
			else {
				light_data.color[i] = glm::vec4(0.0);
			}
			light_data.attenuation[i] = m_point_lights.at(i).attenuation();
		}
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 3, lightSSBO);
